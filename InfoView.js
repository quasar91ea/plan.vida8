                    ...
                    <div className="w-full lg:w-1/3 flex lg:flex-col justify-around gap-4">
                        {phaseKeys.map((key, index) => (
                            <div key={key} onClick={() => setActivePhase(key)} className={`flex-1 flex flex-col items-center justify-center text-center p-4 bg-brand-secondary rounded-lg border border-brand-border cursor-pointer transition-all duration-300 ${activePhase === key ? 'ring-2 ring-brand-accent scale-105' : 'hover:border-brand-tertiary'}`}>
                                ...
                                <h4 className={`font-bold ${activePhase === key ? 'text-white' : 'text-brand-tertiary'}`}>{phasesData[key].title.split(' ')[1]}</h4>
                            </div>
                        ))}
                    </div>
                    <div className="w-full lg:w-2/3 bg-brand-secondary p-6 rounded-lg border border-brand-border min-h-[300px] animate-fade-in">
                        <h3 className="text-xl font-bold text-white mb-3">{phasesData[activePhase].title}</h3>
                        <p className="text-brand-tertiary mb-4">{phasesData[activePhase].description}</p>
                        <ul className="space-y-3 text-sm text-brand-tertiary">
                            {phasesData[activePhase].points.map((p, i) => (
            ...
            <section id="anatomy">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="bg-brand-secondary p-6 rounded-lg border border-brand-border">
                        ...
                        <div className="h-80"><ChartComponent id="gstPrinciplesChart" config={gstPrinciplesConfig} /></div>
                    </div>
                     <div className="bg-brand-secondary p-6 rounded-lg border border-brand-border">
                        <h3 className="text-lg font-semibold text-center mb-4 text-white">Efectividad vs. Bienestar</h3>
                        <div className="h-80"><ChartComponent id="goalEffectivenessChart" config={goalEffectivenessConfig} /></div>
                    </div>
                </div>
            </section>
                    ...
                    <div className="flex flex-wrap justify-center gap-2 mb-6">
                        {Object.keys(tacticsData).map(key => (
                             <button key={key} onClick={() => setActiveTactic(key)} className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors duration-300 ${activeTactic === key ? 'bg-brand-accent text-white' : 'bg-brand-secondary text-brand-tertiary hover:bg-brand-border'}`}>
                                {tacticsData[key].title}
                            </button>
                        ))}
                    </div>
                    <div className="bg-brand-secondary p-6 rounded-lg border border-brand-border min-h-[180px] animate-fade-in">
                        <h3 className="text-lg font-bold text-white mb-2">{tacticsData[activeTactic].title}</h3>
                        <p className="text-sm mb-2"><strong className="text-brand-tertiary">Principio:</strong> {tacticsData[activeTactic].principle}</p>
                        <p className="text-sm mb-2"><strong className="text-brand-tertiary">Aplicación:</strong> {tacticsData[activeTactic].application}</p>
                        <p className="text-xs text-green-400 mb-1">✓ {tacticsData[activeTactic].strength}</p>
                        <p className="text-xs text-red-400">✗ {tacticsData[activeTactic].limit}</p>
                    </div>
                </div>
            </section>
            ...
            <section id="nuances">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="bg-brand-secondary p-6 rounded-lg border border-brand-border">
                        ...
                        <div className="h-80"><ChartComponent id="goalRisksChart" config={goalRisksConfig} /></div>
                    </div>
                     <div className="bg-brand-secondary p-6 rounded-lg border border-brand-border">
                        <h3 className="text-lg font-semibold text-center mb-4 text-white">Motivación y Contexto Cultural</h3>
                        <div className="h-80"><ChartComponent id="culturalMotivationChart" config={culturalMotivationConfig} /></div>
                    </div>
                </div>
            </section>
        </div>

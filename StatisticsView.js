...
const StatisticsView = ({ tasks, goals }) => {
    ...
    const stats = React.useMemo(() => {
        ...
        const goalProgress = goals.map(goal => {
            ...
            const progressByTasks = (completedTasksCount / allGoalTasks.length) * 100;

            if (!goal.dueDate || taskStartDates.length === 0) {
                return { ...goal, progress: progressByTasks };
            }
            ...
            const totalDurationDays = Math.round((goalDueDate.getTime() - goalStartDate.getTime()) / DAY_MS);
            const elapsedDays = Math.round((today.getTime() - goalStartDate.getTime()) / DAY_MS);

            const expectedProgress = (elapsedDays / totalDurationDays) * 100;
            
            return { ...goal, progress: Math.min(progressByTasks, expectedProgress) };
        });
    ...
    const ConsistencyMap = () => {
        ...
        // Pad start for the first week
        for (let i = 0; i < (day.getDay() + 6) % 7; i++) {
            ...
            <div className="overflow-x-auto pb-2">
                <div className="grid grid-rows-7 grid-flow-col gap-1 min-w-[700px]">
                    {days.map((d, i) => (
                        <div
                            key={i}
                            className={`w-full aspect-square rounded-sm border border-transparent transition-all ${getColor(d.count)}`}
